...
namespace Fp.Utility.Stateless
{
    ...
    {
        /// <summary>
        ///     Use immediate mode when the queing of trigger events are not needed. Care must be taken when using this mode,
        ///     as there is no run-to-completion guaranteed.
        /// </summary>
        Immediate,
    ...
    public partial class StateMachine<TState, TTrigger>
    {
        ...
        public StateMachineInfo GetInfo()
        {
            ...
            List<TState> behaviours = _stateConfiguration
                                      .SelectMany(kvp => kvp.Value.TriggerBehaviours.SelectMany(
                                                      b => b.Value.OfType<TransitioningTriggerBehaviour>().Select(tb => tb.Destination))).ToList();
            behaviours.AddRange(_stateConfiguration
                                .SelectMany(kvp => kvp.Value.TriggerBehaviours.SelectMany(
                                                b => b.Value.OfType<ReentryTriggerBehaviour>().Select(tb => tb.Destination))).ToList());
